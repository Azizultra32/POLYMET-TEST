...
const TranscriptHashTask: React.FC<TranscriptHashTaskProps> = ({
  ...
  const hasSummary =
    summaryMap && typeof summaryMap["6"] === "object" && summaryMap["6"] !== null && !Array.isArray(summaryMap["6"])

  return (
    <Card className="h-full flex flex-col">
      <CardHeader className="p-4 pb-2">
        <div className="flex justify-between items-center border-b pb-2">
          <CardTitle className="text-xl font-semibold text-primary-600">#-Task</CardTitle>
      ...
      <CardContent className="flex-grow min-h-0 pt-2 px-4 pb-4">
        {hasSummary ? (
          <TranscriptSummary ref={summaryRef} summary={summaryMap["6"]} transcript={transcript} />
        ) : (
