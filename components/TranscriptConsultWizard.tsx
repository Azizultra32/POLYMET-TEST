...
const TranscriptConsultWizard: React.FC<TranscriptConsultWizardProps> = ({
  ...
  const hasSummary =
    summaryMap && typeof summaryMap["3"] === "object" && summaryMap["3"] !== null && !Array.isArray(summaryMap["3"])

  return (
    <Card className="h-full flex flex-col">
      <CardHeader className="p-4 pb-2">
        <div className="flex justify-between items-center border-b pb-2">
          <CardTitle className="text-xl font-semibold text-primary-600">Consult Wizard</CardTitle>
      ...
      <CardContent className="flex-grow min-h-0 pt-2 px-4 pb-4">
        {hasSummary ? (
          <TranscriptSummary ref={summaryRef} summary={summaryMap["3"]} transcript={transcript} />
        ) : (
